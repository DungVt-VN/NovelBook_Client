            ...
            <div className='book-detail'>
              ...
              <table className="book-detail-table">
                <tbody>
                  ...
                  <tr>
                    <th>Status</th>
                    <td>{BookStatusEnum[book?.status ?? 0]}</td>
                  </tr>
                  <tr>
                    <th>Viewed</th>
                    <td>{book?.viewed}</td>
                  ...
                  <tbody>
                    {
                      (loading &&
                        <tr>
                          <td colSpan={3} className="py-4 text-center text-gray-500"> Loading data ...</td>
                        </tr>
                      )
                    }
                    {
                      (error || bookChapters.length == 0) &&
                      <tr>
                        <td colSpan={3} className="py-4 text-center text-gray-500">Khong co chapter nao!</td>
                      </tr>
                    }
                    {bookChapters && bookChapters.map((chapter, index) => (
                      <tr key={index} onClick={() => handleChapterSelect(chapter.chapterNumber, chapter.chapterId)} className='cursor-pointer'>
                        <td><p className='px-3'>Chapter {chapter.chapterNumber}</p></td>
                        <td className='text-center'>{formatDate(chapter.publishedDate)}</td>
              ...
              <div key={rating.value} className="flex items-center mb-2">
                ...
                <div className="flex-1 h-6 bg-gray-200 mr-2 relative max-w-40">
                  <div
                    className="h-full"
                    style={{
                      width: `${(rating.votes / (book?.voted ?? 5)) * 100}%`,
